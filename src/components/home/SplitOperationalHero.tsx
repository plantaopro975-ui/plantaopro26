          ...
          <div className="relative flex items-end justify-center sm:justify-center min-h-[146px] min-[390px]:min-h-[162px] sm:min-h-[clamp(90px,14vh,220px)] lg:min-h-[clamp(220px,30vh,320px)] xl:min-h-[clamp(260px,34vh,380px)] md:order-none z-[90] overflow-visible pb-0 mb-0 sm:-mb-2 pt-1 sm:pt-0 px-2 sm:px-0">
            ...
            >
              ...
              <picture className="relative z-50 block h-full leading-[0] flex items-end -ml-2 sm:ml-0">
                ...
                {/* CS Gas — granada SVG profissional + fumaça sutil (CSS) */}
                <span
                  aria-hidden
                  className="pointer-events-none absolute bottom-0 right-[-16%] sm:right-[-20%] lg:right-[-16%] w-[56%] sm:w-[50%] lg:w-[44%] h-[46%] sm:h-[50%] overflow-visible z-[45]"
                >
                  {/* Fumaça sutil (só se houver motion) */}
                  ...
                  <span className="absolute left-[8%] right-[52%] bottom-[2%] h-[7%] rounded-[50%] bg-black/55 blur-[3px]" />

                  {/* Granada CS — SVG profissional, deitada no chão */}
                  <svg
                    viewBox="0 0 200 120"
                    className="absolute left-[6%] bottom-[3%] w-[42%] sm:w-[38%] lg:w-[34%] h-auto drop-shadow-[0_6px_10px_rgba(0,0,0,0.75)]"
                    role="img"
                    aria-label="Granada de gás CS"
                  >
                    <defs>
                      <linearGradient id="csBody" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%"  stopColor="#5a6b3a" />
                        <stop offset="35%" stopColor="#3f4d24" />
                        <stop offset="55%" stopColor="#2f3a18" />
                        <stop offset="100%" stopColor="#1a2210" />
                      </linearGradient>
                      <linearGradient id="csCap" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#8a8f7a" />
                        <stop offset="60%" stopColor="#4a4f42" />
                        <stop offset="100%" stopColor="#2b2e26" />
                      </linearGradient>
                      <linearGradient id="csTop" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#c9ccbe" />
                        <stop offset="100%" stopColor="#6b6f60" />
                      </linearGradient>
                      <radialGradient id="csHole" cx="0.5" cy="0.5" r="0.5">
                        <stop offset="0%" stopColor="#0a0a08" />
                        <stop offset="100%" stopColor="#1e2016" />
                      </radialGradient>
                    </defs>

                    {/* Corpo cilíndrico (deitado) */}
                    <g transform="translate(6,32)">
                      {/* corpo principal */}
                      <rect x="14" y="0" width="130" height="56" rx="6" fill="url(#csBody)" stroke="#0e1408" strokeWidth="1.2" />
                      {/* highlight superior */}
                      <rect x="16" y="3" width="126" height="9" rx="4" fill="rgba(255,255,255,0.10)" />
                      {/* sombra inferior */}
                      <rect x="16" y="44" width="126" height="10" rx="4" fill="rgba(0,0,0,0.35)" />

                      {/* faixas normativas (amarela = CS) */}
                      <rect x="14" y="16" width="130" height="5" fill="#e6b800" />
                      <rect x="14" y="35" width="130" height="5" fill="#e6b800" />
                      <rect x="14" y="16" width="130" height="1" fill="rgba(0,0,0,0.4)" />
                      <rect x="14" y="39" width="130" height="1" fill="rgba(0,0,0,0.4)" />

                      {/* stencil "CS" */}
                      <text x="79" y="32" textAnchor="middle" fontFamily="'IBM Plex Mono', ui-monospace, monospace"
                            fontSize="11" fontWeight="800" fill="#f4f1e6" letterSpacing="1.5">CS · 40</text>

                      {/* rebites */}
                      {[24, 60, 96, 132].map((x) => (
                        <circle key={x} cx={x} cy="49" r="1.1" fill="#0a0a08" />
                      ))}

                      {/* Tampa esquerda (base) */}
                      <rect x="4" y="4" width="12" height="48" rx="3" fill="url(#csCap)" stroke="#0e1408" strokeWidth="1" />
                      <line x1="10" y1="6" x2="10" y2="50" stroke="rgba(0,0,0,0.5)" strokeWidth="0.6" />

                      {/* Tampa direita (topo com furos de emissão) */}
                      <rect x="142" y="4" width="14" height="48" rx="3" fill="url(#csTop)" stroke="#0e1408" strokeWidth="1" />
                      {/* furos de emissão */}
                      {[12, 22, 32, 42].map((y) => (
                        <circle key={y} cx="149" cy={y - 4} r="2.2" fill="url(#csHole)" stroke="#0a0a08" strokeWidth="0.5" />
                      ))}

                      {/* Anel de segurança */}
                      <g transform="translate(158,26)">
                        <circle cx="0" cy="0" r="4.5" fill="none" stroke="#c9ccbe" strokeWidth="1.6" />
                        <rect x="-1.2" y="-14" width="2.4" height="10" fill="#8a8f7a" />
                      </g>
                    </g>

                    {/* brilho especular sutil */}
                    <ellipse cx="60" cy="42" rx="34" ry="3" fill="rgba(255,255,255,0.08)" />
                  </svg>
                </span>
              </picture>
            </div>
